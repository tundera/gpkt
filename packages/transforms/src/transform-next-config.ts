...
export type TransformNextConfigCallback = (config: j.ObjectExpression) => j.ObjectExpression

export function transformNextConfig(
  program: Program,
  transform: TransformNextConfigCallback,
): Program {
  ...
  if (moduleExportsExpressions.length === 0) {
    ...
    if (config) {
      ...
      console.warn(
        "The configuration couldn't be found, but there is a 'module.exports' inside `next.config.js`",
      )
    }
  } else {
    console.warn("There are multiple 'module.exports' inside 'next.config.js'")
  }
